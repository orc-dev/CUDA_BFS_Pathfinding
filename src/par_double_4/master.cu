...
void run_test(const std::string map_file, const std::string out_file,
    ...
    // warm-up run
    init_omap(omap, src_ids[0], tgt_ids[0], buf_size);
    measure_gpu_runtime(emap, omap);
    ...
    for (size_t i = 0; i < src_ids.size(); ++i) {
        ...
        init_omap(omap, src_ids[i], tgt_ids[i], buf_size);
        gpu_time = measure_gpu_runtime(emap, omap);
...
int main(int argc, char *argv[]) {
    // pathfinding on sparse map
    {
        ...
        const std::string out_file = "../out/path_S4_DW";
    ...
    // pathfinding on dense map
    {
        ...
        const std::string out_file = "../out/path_D4_DW";
