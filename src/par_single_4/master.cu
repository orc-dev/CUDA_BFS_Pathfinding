 ...
 *          and set the source node with 0.
 *
 * @param   omap     Pointer to the integer array
 * @param   src_id   ID of the source node
 * @param   length   Length of the array
 */
void init_omap(int* omap, const int src_id, const int length) {
    std::fill(omap, omap + length, DEF_VAL);
    omap[src_id] = 0;
}
 ...
 */
float measure_runtime(const std::uint8_t* emap, int* omap, const int tgt_id) {
    // event creation
    float gpu_time;
 ...
 */
double timing_path_rebuild(const int* omap, const int src_id, 
    const int tgt_id, std::vector<std::vector<int>>& path) {
...
void run_test(const std::string map_file, const std::string out_file,
    ...
    cudaMallocManaged(&omap, NSQ * sizeof(int) * 2);
    ...
    // warm-up run
    init_omap(omap, src_ids[0], NSQ * 2);
    measure_runtime(emap, omap, tgt_ids[0]);
    ...
    for (size_t i = 0; i < src_ids.size(); ++i) {
        init_omap(omap, src_ids[i], NSQ * 2);
        gpu_time = measure_runtime(emap, omap, tgt_ids[i]);
...
int main(int argc, char *argv[]) {
    // pathfinding on sparse map
    {
        ...
        const std::string out_file = "../out/path_S4_SW";
